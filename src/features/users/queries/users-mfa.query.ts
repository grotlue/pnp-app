...
export async function getAdminMfaStatus(
  ...
  const response = await apiRequest<AdminMfaStatusResponse>("/api/auth/mfa/totp", { session });
...
export async function enrollAdminTotp(
  ...
  const response = await apiRequest<AdminMfaEnrollResponse>("/api/auth/mfa/totp", {
    method: "POST",
    session,
    body: input ?? {},
  });
...
export async function verifyAdminTotp(
  ...
  const response = await apiRequest<AdminMfaVerifyResponse>("/api/auth/mfa/totp", {
    method: "PATCH",
    session,
    body: input,
  });
