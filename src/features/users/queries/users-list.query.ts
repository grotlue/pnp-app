...
export async function getUsersList(session: ClientSession): Promise<UserListEntry[]> {
  const response = await apiRequest<UserListEntry[]>("/api/users?limit=1000", { session });
