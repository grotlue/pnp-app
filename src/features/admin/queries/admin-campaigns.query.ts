...
export async function listAdminCampaigns(session: ClientSession): Promise<AdminCampaign[]> {
  const response = await apiRequest<AdminCampaign[]>("/api/admin/campaigns", { session });
...
export async function updateAdminCampaign(
  ...
  const response = await apiRequest<AdminCampaign>(`/api/admin/campaigns/${campaignId}`, {
    method: "PATCH",
    session,
    body: input,
  });
...
export async function deleteAdminCampaign(
  ...
  const response = await apiRequest<{ deleted: true }>(`/api/admin/campaigns/${campaignId}`, {
    method: "DELETE",
    session,
  });
