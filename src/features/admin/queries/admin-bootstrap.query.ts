...
import type { AdminCampaign, AdminCharacter, AdminUser } from "@/features/admin/types";
...
export async function getAdminBootstrap(
  ...
  const response = await apiRequest<AdminBootstrapResponse>("/api/admin/bootstrap", { session });
